<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input v-model="listQuery.code" placeholder="Ülke Kodu" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter" />
      <el-input v-model="listQuery.name" placeholder="Ülke Adı" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter" />
    ...
    >
      ...
      <el-table-column label="Kod" width="110px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.code }}</span>
        </template>
      </el-table-column>

      <el-table-column label="Ad" width="110px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.name }}</span>
        </template>
      </el-table-column>
...
<script>
import { fetchPv,fetchCountryList, fetchCountrySearchList ,createCountry, updateCountry, deleteCountry, exportCountryExcel, exportCountryExcelByParameters } from '@/api/article'
...
export default {
  ...
  data() {
    return {
      ...
      pvData: [],
      rules: {
        name: [{ required: true, message: 'Ülke adı zorunludur.', trigger: 'change' }],
        code: [{ required: true, message: 'Ülke kodu zorunludur', trigger: 'change' }],
  ...
  methods: {
    ...
    getSearchList() {
      ...
      fetchCountrySearchList(this.listQuery).then(response => {
        console.log(response)
        this.list = response.map(v => {
            v.id = v.id
            return v
